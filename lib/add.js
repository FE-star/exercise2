function add(num1, num2) {
  ...
  console.log(result);
  return  result + ''; */
  /* let num1Arr = num1.split('').reverse(), // 把num1拆开, 反转
  ...
  let num1Arr = num1.split(''),
      num2Arr = num2.split(''),
      num1Len = num1Arr.length,
      num2Len = num2Arr.length,
      maxLen = Math.max(num1Len, num2Len),
      resultArr = [],
      carry = 0;
  for(let i = 1; i <= maxLen; i++){
    let r = (+num1Arr[num1Len - i]||0) + (+num2Arr[num2Len - i]||0) + carry;
    carry = Math.floor(r / 10);
    resultArr.unshift(r % 10);
  }
  if(carry > 0){
